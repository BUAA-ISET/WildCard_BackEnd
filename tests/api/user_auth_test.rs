...
fn multiple_different_users_can_register_successfully() {
    ...
    let response1 = app.register_user("user1");
    assert_eq!(response1.status_code, 201);
    
    let response2 = app.register_user("user2");
    assert_eq!(response2.status_code, 201);
    
    let response3 = app.register_user("user3");
    assert_eq!(response3.status_code, 201);
}

#[test]
fn user_payload_fields_are_correct() {
    let user = mock::test_data::register_user_payload();
