...
pub async fn list_history(
    ...
    let guard = replay_store.read().await;
...
pub async fn get_replay(
    ...
    let player_id = user_id.to_string();
    let guard = replay_store.read().await;
    let replay = guard.replays.get(&replay_id).ok_or(AppError::NotFound)?;
...
pub async fn start_match_replay(replay_store: &ReplayStore, session: &GameSession, room: &Room) {
    ...
    guard.replays.insert(replay_id, replay);
}

pub async fn append_match_replay_frame(
